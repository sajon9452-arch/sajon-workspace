...
export const BulkMeetingSmsModal: React.FC<BulkMeetingSmsModalProps> = ({
  ...
  const isExecutiveMeeting = !meetingType?.includes('সাধারণ') && (
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const built = buildMeetingRecipients(meetingType, members);
  ...
  const handleToggleSelectAll = () => {
    const allSelected = recipients.every(r => (r.isValidPhone ? r.selected : true));
    setRecipients(prev => prev.map(r => ({
      ...r,
      selected: r.isValidPhone ? !allSelected : false
    })));
  };
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={`inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full border ${
                  ...
                  {isExecutiveMeeting ? 'কার্যকরী কমিটি মিটিং' : 'যৌথ সাধারণ সভা'}
                </span>
              </div>
              <p className="text-xs text-slate-500 mt-0.5">
                সিলেট মানবসেবা সংগঠন • স্বয়ংক্রিয় দ্রুত এসএমএস বিতরণ সিস্টেম
              </p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto space-y-4 py-3.5 pr-1 flex-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center justify-between gap-1">
                <h4 className="text-xs sm:text-sm font-bold">
                  {isExecutiveMeeting 
                    ? 'টার্গেট অডিয়েন্স: শুধুমাত্র কার্যকরী কমিটির সদস্যবৃন্দ' 
                    : 'টার্গেট অডিয়েন্স: কার্যকরী কমিটি ও সাধারণ সদস্য উভয়ই (সকল সদস্য)'}
                </h4>
              ...
              <p className="text-[11px] sm:text-xs text-slate-600 mt-1 leading-relaxed">
                {isExecutiveMeeting 
                  ? `সিস্টেমটি স্বয়ংক্রিয়ভাবে সংগঠনের সংরক্ষিত শুধুমাত্র কার্যকরী কমিটির সদস্যদের ফিল্টার করে মোট ${toBengaliNumber(recipients.length)} জনের তালিকা প্রস্তুত করেছে।` 
                  : `সংগঠনের কার্যকরী কমিটি ও সাধারণ সদস্য উভয়ের যৌথ সভার জন্য মোট ${toBengaliNumber(recipients.length)} জন সদস্যকে টার্গেট করা হয়েছে।`}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-2xl bg-slate-50 border border-slate-200">
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-bold text-slate-700 hover:text-blue-700 transition cursor-pointer"
              >
                {selectedRecipients.length === recipients.filter(r => r.isValidPhone).length ? (
                  <CheckSquare className="w-4 h-4 text-blue-600" />
                ) : (
                  <EmptySquare className="w-4 h-4 text-slate-400" />
                )}
                <span>সবাইকে নির্বাচন ({toBengaliNumber(totalSelected)})</span>
              </button>
            </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between text-xs font-bold text-slate-700 px-1">
              <span>টার্গেট প্রাপক সদস্যদের তালিকা ({toBengaliNumber(recipients.length)})</span>
              <span className="text-[11px] font-normal text-slate-500">
                নির্বাচিতদের একে একে স্বয়ংক্রিয়ভাবে মেসেজ পাঠানো হবে
              </span>
            </div>
